  ...
  render() {
    ...
    return <div className="section has-text-centered">
      ...
      {breedingRequestsOutgoing.length > 0
        ? <button onClick={()=>this.toggleShow('showIncoming')} className={`button is-large is-fullwidth  column is-6 is-offset-3 is-${showOutgoing ? 'warning': 'info'}`}>
          {showOutgoing ? 'Hide Incoming Requests' : 'View Incoming Requests'}
      ...
      <hr />
      {breedingRequestsOutgoing.length > 0
        ? <button className={`button is-large is-fullwidth  column is-6 is-offset-3 is-${showOutgoing ? 'warning': 'info'}`}>
