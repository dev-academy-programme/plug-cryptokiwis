...
class DanceOffRequest extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      showStats: false
  ...
  render() {
    ...
    return <div className="box has-text-centered">
      <h3 className="title is-3">{senderKiwi.name} wants to Dance!</h3>
      <h3 className="subtitletitle is-3">{isIncoming ? `<-- Request ID: ${request.id}` : `Request ID: ${request.id} -->`}</h3>
...
export default connect(mapStateToProps)(DanceOffRequest)
